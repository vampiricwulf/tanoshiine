...
function reload_scripts(cb) {
	async.parallel({
		client: function(callback){
			getScriptRevision('client', callback);
		},
		vendor: function(callback){
			getScriptRevision('vendor', callback);
		},
		mod: function(callback){
			getScriptRevision('mod', callback);
		}
	}, function(err, js){
		if (err)
			return cb(err);
		...
		fs.readFile(path.join('state', js.mod), 'UTF-8', function (err, modSrc) {
			if (err)
				return cb(err);
			RES.modJs = modSrc;
			cb(null);
...
// Read JSON file in ./state, generated by grunt-rev
